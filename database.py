...
def get_candidate(source_file: str, db_path: str | Path = DEFAULT_DB_PATH) -> Optional[dict]:
    """reconstructs the full nested profile by joining all child tables."""
    with get_connection(db_path) as conn:
        candidate = conn.execute("SELECT * FROM candidates WHERE source_file = ?", (source_file,)).fetchone()
        if not candidate:
            return None
        ...
        result["skills"] = [
            dict(r) for r in conn.execute(
                "SELECT name, category, years_experience FROM skills WHERE candidate_source_file = ?", (source_file,),
            ).fetchall()
        ]
        result["work_history"] = [
            dict(r) for r in conn.execute(
                "SELECT company, title, start_date, end_date, is_current, location, responsibilities "
                "FROM work_history WHERE candidate_source_file = ?", (source_file,),
            ).fetchall()
        ]
        result["education"] = [
            dict(r) for r in conn.execute(
                "SELECT institution, degree, field_of_study, start_date, end_date, gpa "
                "FROM education WHERE candidate_source_file = ?", (source_file,),
            ).fetchall()
        ]
        result["keywords"] = [
        ...
        result["extraction_warnings"] = [
            r["warning"] for r in conn.execute(
                "SELECT warning FROM extraction_warnings WHERE candidate_source_file = ?", (source_file,),
            ).fetchall()
        ]
        

def list_candidates(limit: int = 50, dp_path: str | Path = DEFAULT_DB_PATH) -> list[dict]:
    """brief list - candidate rows only, no child-table joins (fast browsing)."""
    with get_connection(db_path) as conn:
        rows = conn.execute(
...
def search_candidates(
    ...
    """real relational database: JOINs against skills table instead of text-matching, returns *distinct* candidates."""
    joins = []
    clauses = []
    params: dict = {}
...
def delete_candidate(source_file: str, db_path: str | Path = DEFAULT_DB_PATH) -> bool:
    ...
    with get_connection(db_path) as conn:
        result = conn.execute("DELETE FROM candidates WHERE source_file = ?", (source_file,))
